...
exports.generateTournamentGroups = void 0;
const client_1 = require("@prisma/client");
const constants_1 = require("../constants");
const MATCH_MATRICES = constants_1.MATCH_MATRIX;
const generateTournamentGroups = async (prisma, tournamentId) => {
    ...
    if (tournament.groupsCreated)
        throw new Error('Los grupos ya han sido generados');
    if (!tournament.numGroup)
        throw new Error('El torneo no tiene definido el número de grupos');
    ...
    const snakeGroups = Array.from({ length: tournament.numGroup }, () => []);
    ...
    await prisma.$transaction(async (tx) => {
        for (let i = 0; i < snakeGroups.length; i++) {
            ...
            const group = await tx.tournamentGroup.create({
                data: {
                    ...
                    group: i + 1,
                    status: client_1.TournamentStatus.Programado,
                },
            });
        ...
        await tx.tournament.update({
            ...
            data: {
                groupsCreated: true,
                status: client_1.TournamentStatus.Grupos,
            },
        });
    });
    return { success: true, message: 'Grupos y partidos generados mediante Serpiente' };
};
exports.generateTournamentGroups = generateTournamentGroups;
