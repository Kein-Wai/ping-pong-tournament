...
exports.fetchTournamentBracket = exports.processKnockoutAdvancement = exports.saveKnockoutBracket = exports.createKnockoutDraw = exports.harvestKnockoutPlayers = void 0;
const client_1 = require("@prisma/client");
const constants_1 = require("../constants");
const match_1 = require("./match");
const crypto_1 = require("crypto");
const ROUND_PROGRESSION = [
    client_1.RoundKnockouts.R128avos,
    client_1.RoundKnockouts.R64avos,
    client_1.RoundKnockouts.R32avos,
    client_1.RoundKnockouts.R16avos,
    client_1.RoundKnockouts.Octavos,
    client_1.RoundKnockouts.Cuartos,
    client_1.RoundKnockouts.Semifinales,
    client_1.RoundKnockouts.Final,
];
const determineInitialRound = (numMatches) => {
    switch (numMatches) {
        case 1:
            return client_1.RoundKnockouts.Final;
        case 2:
            return client_1.RoundKnockouts.Semifinales;
        case 4:
            return client_1.RoundKnockouts.Cuartos;
        case 8:
            return client_1.RoundKnockouts.Octavos;
        case 16:
            return client_1.RoundKnockouts.R16avos;
        case 32:
            return client_1.RoundKnockouts.R32avos;
        case 64:
            return client_1.RoundKnockouts.R64avos;
        case 128:
            return client_1.RoundKnockouts.R128avos;
        default:
...
const generateStandardSeedPattern = (size) => {
    let seeds = [1, 2];
...
const harvestKnockoutPlayers = async (prisma, tournamentId) => {
    ...
    if (groupIds.length === 0)
        throw new Error('No hay grupos en este torneo');
    ...
    for (const clas of classifications) {
        if (clas.position > 0 && clas.position <= tournament.playersKnockout) {
            ...
            if (tournament.typeKnockout === client_1.TypeKnockout.LlaveAB) {
                bracketB.push({
                    ...clas,
                    groupNumber: groups.find((g) => g.id === clas.tournamentGroupId)?.group,
                });
...
exports.harvestKnockoutPlayers = harvestKnockoutPlayers;
const createKnockoutDraw = (classifiedPlayers, sortType, allPos) => {
    const numPlayers = classifiedPlayers.length;
    if (numPlayers < 2)
        throw new Error('No hay suficientes jugadores para eliminatorias');
    const bracketSize = getNextPowerOfTwo(numPlayers);
    const drawSlots = new Array(bracketSize).fill(null);
    if (sortType === client_1.SortKnockout.Aleatorio) {
        ...
        const seededPlayers = [];
        if (rankedPlayers[0])
            seededPlayers[1] = rankedPlayers[0];
        if (rankedPlayers[1])
            seededPlayers[2] = rankedPlayers[1];
        const tier3_4 = shuffle(rankedPlayers.slice(2, 4));
        if (tier3_4[0])
            seededPlayers[3] = tier3_4[0];
        if (tier3_4[1])
            seededPlayers[4] = tier3_4[1];
...
exports.createKnockoutDraw = createKnockoutDraw;
const saveKnockoutBracket = async (prisma, tournamentId, type, firstRoundMatches, dateStart, allPos = false) => {
    if (firstRoundMatches.length === 0)
        throw new Error('No hay partidos para guardar.');
    const bracketSize = firstRoundMatches.length * 2;
    const initialRoundIndex = ROUND_PROGRESSION.indexOf(determineInitialRound(firstRoundMatches.length));
    let currentBrackets = [
        {
            matches: firstRoundMatches.map((m, idx) => {
                ...
                return {
                    ...
                    p1Id: m.playerOne ? m.playerOne.playerId : constants_1.BYE_USER_ID,
                    p2Id: m.playerTwo ? m.playerTwo.playerId : constants_1.BYE_USER_ID,
                    matchOrder: idx,
                    winnerGoesToMatchId: null,
                    loserGoesToMatchId: null,
                    status: isBye ? client_1.MatchStatus.Completado : client_1.MatchStatus.Programado,
                    isBye,
                };
            }),
            posStart: 1,
    ...
    while (currentBrackets.length > 0) {
        ...
        for (const bracket of currentBrackets) {
            ...
            for (let i = 0; i < bracket.matches.length; i += 2) {
                ...
                const wMatch = {
                    id: (0, crypto_1.randomUUID)(),
                    p1Id: constants_1.TBD_USER_ID,
                    p2Id: constants_1.TBD_USER_ID,
    ...
    await prisma.$transaction(async (tx) => {
        for (const bracket of allGeneratedBrackets) {
            ...
            const matchesData = bracket.matches.map((m) => {
                if (m.isBye)
                    byeMatchesIdsToTrigger.push(m.id);
                return {
                    id: m.id,
                    dateStart,
                    tournamentId,
                    ...
                    status: m.status,
                    setOnePlayerOne: m.isBye && m.p1Id !== constants_1.BYE_USER_ID ? 11 : 0,
                    setOnePlayerTwo: m.isBye && m.p2Id !== constants_1.BYE_USER_ID ? 11 : 0,
        ...
        if (type === client_1.KnockoutType.A) {
            // ✅ Usamos initialRoundIndex, que se calculó al principio de la función
            await tx.tournament.update({
                where: { id: tournamentId },
                data: { status: ROUND_PROGRESSION[initialRoundIndex] },
            });
        }
    });
    for (const byeMatchId of byeMatchesIdsToTrigger) {
        await (0, exports.processKnockoutAdvancement)(prisma, byeMatchId);
    }
    return { success: true };
};
exports.saveKnockoutBracket = saveKnockoutBracket;
// Añade esta función helper justo encima de processKnockoutAdvancement
const recordTournamentClassification = async (tx, match, playerId, isWinner) => {
    // Evitamos registrar a fantasmas o TBDs
    if (playerId === constants_1.BYE_USER_ID || playerId === constants_1.TBD_USER_ID)
        return;
...
const processKnockoutAdvancement = async (prisma, matchId) => {
    ...
    if (!completedMatch || completedMatch.status !== 'Completado' || !completedMatch.knockout)
        return;
    const matchResults = (0, match_1.calculateMatchResults)(completedMatch);
...
exports.processKnockoutAdvancement = processKnockoutAdvancement;
const fetchTournamentBracket = async (prisma, tournamentId, type = client_1.KnockoutType.A) => {
...
exports.fetchTournamentBracket = fetchTournamentBracket;
