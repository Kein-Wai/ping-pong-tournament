...
async function main() {
    ...
    // --- 1. ROLES ---
    const types = [
        { name: client_1.TypeUser.SuperAdmin },
        { name: client_1.TypeUser.AdminClub },
        { name: client_1.TypeUser.Player },
    ];
    let savedTypes = [];
    for (const type of types) {
        savedTypes.push(await prisma.userType.upsert({
    ...
    const hashedPasswordSuper = await bcryptjs_1.default.hash('112233cheung', 10);
    ...
    const hashedPasswordClub = await bcryptjs_1.default.hash('112233club', 10);
    ...
    async function simulateAndSaveMatch(tournamentId, p1Id, p2Id, groupId = null, knockoutId = null, clas1Id = null, clas2Id = null, matchOrder = 0) {
        let p1SetsWon = 0;
        let p2SetsWon = 0;
        let ptsP1 = 0;
        let ptsP2 = 0;
    ...
    const t1Groups = [];
    for (let i = 1; i <= 8; i++) {
        t1Groups.push(await prisma.tournamentGroup.create({
            data: { tournamentId: t1.id, group: i, status: 'Completado' },
        }));
    }
    const snakeGroupsT1 = Array.from({ length: 8 }, () => []);
    ...
    for (let g = 0; g < 8; g++) {
        ...
        for (let i = 0; i < groupPlayers.length; i++) {
            for (let j = i + 1; j < groupPlayers.length; j++) {
                await simulateAndSaveMatch(t1.id, groupPlayers[i].id, groupPlayers[j].id, groupDb.id, null, clasRecords[i].clasId, clasRecords[j].clasId);
            }
        }
    }
    const advancingT1 = [];
    ...
    for (let i = 0; i < 16; i += 2) {
        const res = await simulateAndSaveMatch(t1.id, advancingT1[bracket16[i]], advancingT1[bracket16[i + 1]], null, kOctavosT1.id, null, null, i / 2);
    ...
    for (let i = 0; i < 8; i += 2) {
        const res = await simulateAndSaveMatch(t1.id, octavosMatchesT1[i].winner, octavosMatchesT1[i + 1].winner, null, kCuartosT1.id, null, null, i / 2);
    ...
    for (let i = 0; i < 4; i += 2) {
        const res = await simulateAndSaveMatch(t1.id, cuartosMatchesT1[i].winner, cuartosMatchesT1[i + 1].winner, null, kSemisT1.id, null, null, i / 2);
    ...
    const finalMatchT1 = await simulateAndSaveMatch(t1.id, semisMatchesT1[0].winner, semisMatchesT1[1].winner, null, kFinalT1.id, null, null, 0);
    await prisma.match.update({
    ...
    const t3Groups = [];
    for (let i = 1; i <= 4; i++) {
        t3Groups.push(await prisma.tournamentGroup.create({
            data: { tournamentId: t3.id, group: i, status: 'Completado' },
        }));
    }
    const snakeGroupsT3 = Array.from({ length: 4 }, () => []);
    ...
    for (let g = 0; g < 4; g++) {
        ...
        for (let i = 0; i < groupPlayers.length; i++) {
            for (let j = i + 1; j < groupPlayers.length; j++) {
                await simulateAndSaveMatch(t3.id, groupPlayers[i].id, groupPlayers[j].id, groupDb.id, null, clasRecords[i].clasId, clasRecords[j].clasId);
            }
        }
    }
    const advancingT3 = [];
    ...
    const t4Groups = [];
    for (let i = 1; i <= 4; i++) {
        t4Groups.push(await prisma.tournamentGroup.create({
            data: { tournamentId: t4.id, group: i, status: 'Completado' },
        }));
    }
    const snakeGroupsT4 = Array.from({ length: 4 }, () => []);
    ...
    for (let g = 0; g < 4; g++) {
        ...
        for (let i = 0; i < groupPlayers.length; i++) {
            for (let j = i + 1; j < groupPlayers.length; j++) {
                await simulateAndSaveMatch(t4.id, groupPlayers[i].id, groupPlayers[j].id, groupDb.id, null, clasRecords[i].clasId, clasRecords[j].clasId);
            }
        }
    }
    const advancingT4 = [];
    ...
    for (let i = 0; i < 8; i += 2) {
        const res = await simulateAndSaveMatch(t4.id, advancingT4[bracket8[i]], advancingT4[bracket8[i + 1]], null, kCuartosT4.id, null, null, i / 2);
    ...
    for (let i = 0; i < 4; i += 2) {
        const res = await simulateAndSaveMatch(t4.id, cuartosMatchesT4[i].winner, cuartosMatchesT4[i + 1].winner, null, kSemisT4.id, null, null, i / 2);
    ...
    const finalMatchT4 = await simulateAndSaveMatch(t4.id, semisMatchesT4[0].winner, semisMatchesT4[1].winner, null, kFinalT4.id, null, null, 0);
    await prisma.match.update({
    ...
    const t5Groups = [];
    for (let i = 1; i <= 4; i++) {
        t5Groups.push(await prisma.tournamentGroup.create({
            data: { tournamentId: t5.id, group: i, status: 'Programado' },
        }));
    }
    // 2. Distribuir en Snake
    const snakeGroupsT5 = Array.from({ length: 4 }, () => []);
    ...
    async function simulateExistingMatch(matchId, p1Id, p2Id, clas1Id, clas2Id) {
        let p1SetsWon = 0, p2SetsWon = 0, ptsP1 = 0, ptsP2 = 0;
        const setScores = [];
